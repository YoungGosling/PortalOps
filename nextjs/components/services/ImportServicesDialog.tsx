...
export function ImportServicesDialog({
  ...
  const handleImport = async () => {
    ...
    try {
      setUploading(true);
      const result = await importServicesAction(selectedFile);
      setImportResult(result);
      
      if (result.success_count > 0) {
        toast.success(`Successfully imported ${result.success_count} service(s)`);
        if (result.failed_count > 0) {
          toast.warning(`${result.failed_count} service(s) failed to import`);
        }
        onSuccess();
      } else {
        toast.error('No services were imported. Please check the errors.');
      }
    } catch (error: any) {
